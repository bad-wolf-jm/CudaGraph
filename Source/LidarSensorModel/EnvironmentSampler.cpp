...
namespace LTSE::SensorModel
{
    ...
    {
        uint32_t lLayers = aLeft.Get<sMultiTensorComponent>().Shape().CountLayers();

        sTensorShape &lXShape = aLeft.Get<sMultiTensorComponent>().Shape();
        sTensorShape &lYShape = aRight.Get<sMultiTensorComponent>().Shape();
    ...
    void EnvironmentSampler::CreateGraph()
    {
        ...
        else if( mSpec.mMultiSamplingFactor > 1 )
        {
            ...
            auto lAzimuthSamplingCoefficients   = MultiTensorValue( *mScope, lInitializer, lAzimuthMultiSampledXY0.Get<sMultiTensorComponent>().Shape() );
            auto lElevationSamplingCoefficients = MultiTensorValue( *mScope, lInitializer, lElevationMultiSampledXY0.Get<sMultiTensorComponent>().Shape() );

            mSampledAzimuths   = Mix( *mScope, lAzimuthMultiSampledXY0, lAzimuthMultiSampledXY1, lAzimuthSamplingCoefficients );
            mSampledElevations = Mix( *mScope, lElevationMultiSampledXY0, lElevationMultiSampledXY1, lElevationSamplingCoefficients );
        }
        else
        {
        ...
        mFlashIdLUT = Flatten( mScope->WithOpName( "FlashId" ), mFlashIdLUT );
        mTimestamps = Flatten( mScope->WithOpName( "Timestamp" ), mTimestamps );
    }
