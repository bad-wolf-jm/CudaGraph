...
namespace LTSE::Graphics::Internal
{
    ...
    class VkContext
    {
      public:
        ...
        VkImage CreateImage( uint32_t aWidth, uint32_t aHeight, uint32_t aDepth, uint32_t aMipLevels, uint32_t aLayers, uint8_t aSampleCount, bool aCubeCompatible,
                             VkFormat aFormat, VkMemoryPropertyFlags aProperties, VkImageUsageFlags aUsage );
        void DestroyImage( VkImage aBuffer );

        VkSampler CreateSampler( VkFilter aMinificationFilter, VkFilter aMagnificationFilter, VkSamplerAddressMode aWrappingMode, VkSamplerMipmapMode aMipmapMode );
        void DestroySampler( VkSampler aImage );

        VkImageView CreateImageView( VkImage aImageObject, uint32_t aLayerCount, VkImageViewType aViewType, VkFormat aImageFormat, VkImageAspectFlags aAspectMask,
        ...
        VkFramebuffer CreateFramebuffer( std::vector<VkImageView> aImageViews, uint32_t aWidth, uint32_t aHeight, uint32_t aLayers, VkRenderPass aRenderPass );
        void DestroyFramebuffer( VkFramebuffer aFramebuffer );

        VkPipelineLayout CreatePipelineLayout( std::vector<VkDescriptorSetLayout> aDescriptorSetLayout, std::vector<VkPushConstantRange> aPushConstants );
        ...
        VkDescriptorSet AllocateDescriptorSet( VkDescriptorPool aDescriptorPool, VkDescriptorSetLayout aLayout, uint32_t aDescriptorCount = 0 );
        ...
        template <typename _MapType> _MapType *MapMemory( VkDeviceMemory aMemory, size_t aSize, size_t aOffset )
        {
        ...
        VkResult AcquireNextImage( VkSwapchainKHR aSwapChain, uint64_t aTimeout, VkSemaphore aWaitSemaphore, uint32_t *aNewImageIndex );
