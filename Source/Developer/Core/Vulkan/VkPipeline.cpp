...
namespace LTSE::Graphics::Internal
{
    ...
    sVkDescriptorSetLayoutObject::sVkDescriptorSetLayoutObject( Ref<VkContext> aContext, std::vector<VkDescriptorSetLayoutBinding> aBindings, bool aUnbounded )
        : mContext{ aContext }
    {
    ...
    sVkDescriptorSetObject::sVkDescriptorSetObject( Ref<VkContext> aContext, VkDescriptorSet aDescriporSet )
        : mContext{ aContext }
    ...
    void sVkDescriptorSetObject::Write( sBufferBindInfo aBuffers )
    {
        ...
        lWriteBufferInfo.offset = aBuffers.Offset;
        lWriteBufferInfo.range  = aBuffers.Size;

        lWriteDSOps.sType           = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
        lWriteDSOps.pNext           = nullptr;
        lWriteDSOps.dstBinding      = aBuffers.Binding;
        lWriteDSOps.dstSet          = mVkObject;
        lWriteDSOps.descriptorCount = 1;

        if( aBuffers.Type == eBufferBindType::STORAGE_BUFFER )
        {
            lWriteDSOps.descriptorType = aBuffers.DynamicOffset ? VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC : static_cast<VkDescriptorType>( eDescriptorType::STORAGE_BUFFER );
        }
        else if( aBuffers.Type == eBufferBindType::UNIFORM_BUFFER )
        {
            lWriteDSOps.descriptorType = aBuffers.DynamicOffset ? VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC : static_cast<VkDescriptorType>( eDescriptorType::UNIFORM_BUFFER );
        }
        else
        {
    ...
    sVkDescriptorPoolObject::sVkDescriptorPoolObject( Ref<VkContext> aContext, uint32_t aDescriptorSetCount, std::vector<VkDescriptorPoolSize> aPoolSizes )
        : mContext{ aContext }
    {
    ...
    Ref<sVkDescriptorSetObject> sVkDescriptorPoolObject::Allocate( Ref<sVkDescriptorSetLayoutObject> aLayout, uint32_t aDescriptorCount )
    {
        return LTSE::Core::New<sVkDescriptorSetObject>( mContext, mContext->AllocateDescriptorSet( mVkObject, aLayout->mVkObject, aDescriptorCount ) );
    }

    sVkPipelineLayoutObject::sVkPipelineLayoutObject( Ref<VkContext> aContext, std::vector<Ref<sVkDescriptorSetLayoutObject>> aDescriptorSetLayout,
        ...
        : mContext{ aContext }
    {
        ...
        for( uint32_t i = 0; i < aPushConstantRanges.size(); i++ )
        {
            ...
            lPushConstant.offset     = aPushConstantRanges[i].Offset;
            lPushConstant.size       = aPushConstantRanges[i].Size;
            lPushConstant.stageFlags = (VkShaderStageFlags)aPushConstantRanges[i].ShaderStages;
    ...
    static uint32_t BufferDataTypeSize( eBufferDataType type )
    {
        // clang-format off
        switch( type )
        {
    ...
    sBufferLayoutElement::sBufferLayoutElement( const std::string &name, eBufferDataType type, uint32_t binding, uint32_t location )
        : Name( name )
        , Type( type )
        , Binding( binding )
        , Location( location )
        , Size( BufferDataTypeSize( type ) )
        , Offset( 0 )
    {
    }

    sBufferLayout::sBufferLayout( std::initializer_list<sBufferLayoutElement> elements )
        : mElements( elements )
    {
    ...
    void sBufferLayout::Compile( uint32_t aBinding, VkVertexInputBindingDescription &o_Binding, std::vector<VkVertexInputAttributeDescription> &o_Attributes, bool aInstanced )
    {
        o_Binding.binding   = aBinding;
        o_Binding.stride    = mStride;
        o_Binding.inputRate = aInstanced ? VK_VERTEX_INPUT_RATE_INSTANCE : VK_VERTEX_INPUT_RATE_VERTEX;

        o_Attributes.resize( mElements.size() );
        for( uint32_t i = 0; i < mElements.size(); i++ )
        {
            VkVertexInputAttributeDescription positionAttribute{};
            positionAttribute.binding  = aBinding;
            positionAttribute.location = mElements[i].Location;
            positionAttribute.format   = (VkFormat)mElements[i].Type;
            positionAttribute.offset   = mElements[i].Offset;
            o_Attributes[i]            = positionAttribute;
        }
    }

    sVkPipelineObject::sVkPipelineObject( Ref<VkContext> aContext, uint8_t aSampleCount, sBufferLayout aVertexBufferLayout, sBufferLayout aInstanceBufferLayout,
                                          ePrimitiveTopology aTopology, eFaceCulling aCullMode, float aLineWidth, sDepthTesting aDepthTest, std::vector<sShader> aShaderStages,
        ...
        : mContext{ aContext }
    {
        ...
        lColorBlendAttachment.colorWriteMask      = VK_COLOR_COMPONENT_R_BIT | VK_COLOR_COMPONENT_G_BIT | VK_COLOR_COMPONENT_B_BIT | VK_COLOR_COMPONENT_A_BIT;
        lColorBlendAttachment.blendEnable         = VK_TRUE;
        lColorBlendAttachment.srcColorBlendFactor = VK_BLEND_FACTOR_SRC_ALPHA;
        lColorBlendAttachment.dstColorBlendFactor = VK_BLEND_FACTOR_ONE_MINUS_SRC_ALPHA;
        lColorBlendAttachment.colorBlendOp        = VK_BLEND_OP_ADD;
        lColorBlendAttachment.srcAlphaBlendFactor = VK_BLEND_FACTOR_ONE;
        lColorBlendAttachment.dstAlphaBlendFactor = VK_BLEND_FACTOR_ZERO;
        lColorBlendAttachment.alphaBlendOp        = VK_BLEND_OP_MAX;
        lColorBlendAttachment.colorWriteMask      = VK_COLOR_COMPONENT_R_BIT | VK_COLOR_COMPONENT_G_BIT | VK_COLOR_COMPONENT_B_BIT | VK_COLOR_COMPONENT_A_BIT;
