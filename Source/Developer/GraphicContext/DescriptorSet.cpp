...
namespace LTSE::Graphics
{
    ...
    void DescriptorSet::Write( Ref<Buffer> aBuffer, bool aDynamicOffset, uint32_t aOffset, uint32_t aSize, uint32_t aBinding )
    {
        ...
        lBufferBindInfo.Type          = aBuffer->mType;
        lBufferBindInfo.DynamicOffset = false;
        lBufferBindInfo.Binding       = aBinding;
        lBufferBindInfo.Offset        = aOffset;
        lBufferBindInfo.Size          = aSize;
    ...
    DescriptorBindingInfo::operator VkDescriptorSetLayoutBinding() const
    {
        ...
        if( ShaderStages & Internal::eShaderStageTypeFlags::VERTEX )
            lNewBinding.stageFlags |= VK_SHADER_STAGE_VERTEX_BIT;

        if( ShaderStages & Internal::eShaderStageTypeFlags::COMPUTE )
            lNewBinding.stageFlags |= VK_SHADER_STAGE_COMPUTE_BIT;

        if( ShaderStages & Internal::eShaderStageTypeFlags::GEOMETRY )
            lNewBinding.stageFlags |= VK_SHADER_STAGE_GEOMETRY_BIT;

        if( ShaderStages & Internal::eShaderStageTypeFlags::FRAGMENT )
            lNewBinding.stageFlags |= VK_SHADER_STAGE_FRAGMENT_BIT;

        if( ShaderStages & Internal::eShaderStageTypeFlags::TESSELATION_CONTROL )
            lNewBinding.stageFlags |= VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT;

        if( ShaderStages & Internal::eShaderStageTypeFlags::TESSELATION_EVALUATION )
