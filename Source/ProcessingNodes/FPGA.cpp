...
namespace LTSE::SensorModel
{
    ...
    void sFPGAProcessController::Run()
    {
        ...
        for( auto lSize : lOperandData.mSegmentCount.Get<sU32VectorComponent>().mValue )
            lMaxAPDSize = std::max( lMaxAPDSize, lSize );

        auto &lWaveforms      = lOperandData.mWaveforms.Get<sMultiTensorComponent>().mValue;
        auto &lSegmentCounts  = lOperandData.mSegmentCount.Get<sVectorBufferComponent>().mValue;
        auto &lWaveformLength = lOperandData.mWaveformLength.Get<sVectorBufferComponent>().mValue;
    ...
    {
        uint32_t lLayerCount = aWaveforms.Get<sMultiTensorComponent>().Shape().CountLayers();

        std::vector<std::vector<uint32_t>> lWaveformShape = aWaveforms.Get<sMultiTensorComponent>().Shape().mShape;
