...
TEST_CASE( "LUA SensorModelBase", "[CORE_SCRIPTING]" )
{
    ScriptingEngine lScriptingEngine{};
...
value = Sensor.SensorModel()
    ...
    auto lSensorModelBase = lScriptingEngine.GetSharedPtr<SensorModelBase>( "value" );
    REQUIRE( lSensorModelBase->mName == "" );
}

TEST_CASE( "LUA SensorModelBase CreateEntity", "[CORE_SCRIPTING]" )
{
    ScriptingEngine lScriptingEngine{};

    lScriptingEngine.Execute( R"(
sensor = Sensor.SensorModel()
entity_0 = sensor:create_entity()
entity_1 = sensor:create_entity(entity_0)
entity_2 = sensor:create_entity("entity_2")
entity_3 = sensor:create_entity("entity_3", entity_1)
)" );

    auto lEntity0 = lScriptingEngine.Get<Entity>( "entity_0" );
    REQUIRE( lEntity0.IsValid() );

    auto lEntity1 = lScriptingEngine.Get<Entity>( "entity_1" );
    REQUIRE( lEntity1.IsValid() );
    REQUIRE( ( lEntity1.Has<sRelationshipComponent>() && ( lEntity1.Get<sRelationshipComponent>().mParent == lEntity0 ) ) );

    auto lEntity2 = lScriptingEngine.Get<Entity>( "entity_2" );
    REQUIRE( lEntity2.IsValid() );
    REQUIRE( ( lEntity2.Has<sTag>() && ( lEntity2.Get<sTag>().mValue == "entity_2" ) ) );

    auto lEntity3 = lScriptingEngine.Get<Entity>( "entity_3" );
    REQUIRE( lEntity3.IsValid() );
    REQUIRE( ( lEntity3.Has<sRelationshipComponent>() && ( lEntity3.Get<sRelationshipComponent>().mParent == lEntity1 ) ) );
    REQUIRE( ( lEntity3.Has<sTag>() && ( lEntity3.Get<sTag>().mValue == "entity_3" ) ) );
}

TEST_CASE( "LUA SensorModelBase Create tile", "[CORE_SCRIPTING]" )
{
    ScriptingEngine lScriptingEngine{};

    lScriptingEngine.Execute( R"(
sensor = Sensor.SensorModel()
tile_0 = sensor:create_tile("tile_0", Math.vec2{1.0, 3.0})
)" );

    auto lSensorModelBase = lScriptingEngine.GetSharedPtr<SensorModelBase>( "sensor" );
    REQUIRE( lSensorModelBase->GetTileByID( "tile_0" ).IsValid() );

    auto lTile = lScriptingEngine.Get<Entity>( "tile_0" );
    REQUIRE( lTile.IsValid() );
    REQUIRE( lTile.Has<sTileSpecificationComponent>() );
    REQUIRE( lTile.Has<sRelationshipComponent>() );
    REQUIRE( lTile.Get<sTileSpecificationComponent>().mID == "tile_0" );
    REQUIRE( lTile.Get<sTileSpecificationComponent>().mPosition == math::vec2{ 1.0f, 3.0f } );
}

TEST_CASE( "LUA SensorModelBase Create flash", "[CORE_SCRIPTING]" )
{
    ScriptingEngine lScriptingEngine{};
...
sensor = Sensor.SensorModel()
tile_0 = sensor:create_tile("tile_0", Math.vec2{1.0, 3.0});
flash_0 = sensor:create_flash(tile_0, "", Math.vec2{1.0, 3.0}, Math.vec2{3.0, 4.0});
)" );
    ...
    auto lTile = lScriptingEngine.Get<Entity>( "tile_0" );
    REQUIRE( lTile.Get<sRelationshipComponent>().mChildren.size() == 1 );

    auto lFlash = lScriptingEngine.Get<Entity>( "flash_0" );
    REQUIRE( lFlash.IsValid() );
    REQUIRE( lTile.Get<sRelationshipComponent>().mChildren[0] == lFlash );
}

TEST_CASE( "LUA SensorModelBase load asset", "[CORE_SCRIPTING]" )
{
    ScriptingEngine lScriptingEngine{};

    lScriptingEngine.Execute( R"(
sensor = Sensor.SensorModel()
asset = sensor:load_asset("asset", "asset_name", "Tests/data/TestSensor0", "photodetector/component.yaml");
)" );

    auto lAssetEntity = lScriptingEngine.Get<Entity>( "asset" );
    REQUIRE( lAssetEntity.Has<sPhotoDetector>() );
}

TEST_CASE( "LUA SensorModelBase acquisition context", "[CORE_SCRIPTING]" )
{
    ScriptingEngine lScriptingEngine{};
...
tile_0  = sensor:create_tile("tile_0", Math.vec2{1.0, 3.0});
flash_0 = sensor:create_flash(tile_0, "", Math.vec2{1.0, 3.0}, Math.vec2{3.0, 4.0});
context = Sensor.AcquisitionContext({}, tile_0, Math.vec2{1.2, 3.4}, 12.4)
)" );

    auto lAcquisitionContext = lScriptingEngine.Get<AcquisitionContext>( "context" );
    REQUIRE( true );
}

TEST_CASE( "LUA SensorModelBase environment sampler", "[CORE_SCRIPTING]" )
{
    ScriptingEngine lScriptingEngine{};
...
flash_0 = sensor:create_flash(tile_0, "", Math.vec2{1.0, 3.0}, Math.vec2{3.0, 4.0});
context = Sensor.AcquisitionContext({}, tile_0, Math.vec2{1.2, 3.4}, 12.4)
scope   = Cuda.Scope.ref_new(128 * 1024 * 1024)
sampler = Sensor.EnvironmentSampler({}, scope, context)
sampler:run()
)" );

    auto lEnvironmentSampler = lScriptingEngine.GetSharedPtr<EnvironmentSampler>( "sampler" );
    auto lAcquisitionContext = lScriptingEngine.Get<AcquisitionContext>( "context" );
    REQUIRE( lEnvironmentSampler->GetScheduledFlashCount() == 1 );
}

#if 0
TEST_CASE( "LUA SensorDevice", "[CORE_SCRIPTING]" )
{
    LuaSensorDevice lDevice( 128 * 1024 * 1024, "C:\\GitLab\\LTSimulationEngine\\Tests\\Data\\TestSensor0\\test_sensor_0.lua" );

    uint32_t lAssetCount = 0;
    lDevice.mSensorDefinition->ForEach<sAssetMetadata>( [&]( auto e, auto &c ) { lAssetCount++; } );
    REQUIRE( lAssetCount == 2 );

    uint32_t lTileCount = 0;
    lDevice.mSensorDefinition->ForEach<sTileSpecificationComponent>( [&]( auto e, auto &c ) { lTileCount++; } );
    REQUIRE( lTileCount == 2 );

    uint32_t lFlashCount = 0;
    lDevice.mSensorDefinition->ForEach<sLaserFlashSpecificationComponent>( [&]( auto e, auto &c ) { lFlashCount++; } );
    REQUIRE( lFlashCount == 3 );

    auto lSensorModel = lDevice.mSensorDefinition;
...
TEST_CASE( "LUA SensorDevice sampling", "[CORE_SCRIPTING]" )
{
    LuaSensorDevice lDevice( 128 * 1024 * 1024, "C:\\GitLab\\LTSimulationEngine\\Tests\\Data\\TestSensor0\\test_sensor_0.lua" );
    ...
    auto lSampler = lDevice.Sample( lAcqCreateInfo, lCreateInfo, std::vector<Entity>{ lDevice.mSensorDefinition->GetTileByID( "tile_1" ) },
                                    std::vector<math::vec2>{ math::vec2{ 1.0f, 2.0f } }, std::vector<float>{ 3.0f } );

    REQUIRE( ( *lSampler )["Azimuth"].IsValid() );
    REQUIRE( ( *lSampler )["Elevation"].IsValid() );
    REQUIRE( ( *lSampler )["Intensity"].IsValid() );
    REQUIRE( ( *lSampler )["FlashId"].IsValid() );
    REQUIRE( ( *lSampler )["Timestamp"].IsValid() );

    {
        std::vector<float> lAzimuths0 = ( *lSampler )["Azimuth"].Get<sMultiTensorComponent>().mValue.FetchFlattened<float>();
    ...
    {
        std::vector<float> lElevations0 = ( *lSampler )["Elevation"].Get<sMultiTensorComponent>().mValue.FetchFlattened<float>();
