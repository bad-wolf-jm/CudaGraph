...
class TestEnvironmentSampler : public EnvironmentSampler
{
  public:
    ...
    sMultiTensorComponent &Retrieve( std::string aName ) { return ( *mScope )[aName].Get<sMultiTensorComponent>(); }
...
TEST_CASE( "Environment sampling", "[CORE_SENSOR_MODEL]" )
{
    ...
    SECTION( "Create environment sampler" )
    {
        ...
        auto lAzimuths = lSampler.GetAzimuths().Get<sMultiTensorComponent>();
        REQUIRE( lAzimuths.Shape().CountLayers() == lExpectedTensorLayers );
        REQUIRE( lAzimuths.Shape().mRank == lExpectedTensorRank );
        REQUIRE( lAzimuths.Shape().mShape == lExpectedTensorShape.mShape );

        auto lElevations = lSampler.GetElevations().Get<sMultiTensorComponent>();
        REQUIRE( lElevations.Shape().CountLayers() == lExpectedTensorLayers );
        REQUIRE( lElevations.Shape().mRank == lExpectedTensorRank );
        REQUIRE( lElevations.Shape().mShape == lExpectedTensorShape.mShape );

        auto lIntensities = lSampler.GetIntensities().Get<sMultiTensorComponent>();
        REQUIRE( lIntensities.Shape().CountLayers() == lExpectedTensorLayers );
        REQUIRE( lIntensities.Shape().mRank == lExpectedTensorRank );
        REQUIRE( lIntensities.Shape().mShape == lExpectedTensorShape.mShape );

        auto lFlashID = lSampler.GetFlashID().Get<sMultiTensorComponent>();
        REQUIRE( lFlashID.Shape().CountLayers() == lExpectedTensorLayers );
        REQUIRE( lFlashID.Shape().mRank == lExpectedTensorRank );
        REQUIRE( lFlashID.Shape().mShape == lExpectedTensorShape.mShape );

        auto lTimestamps = lSampler.GetTimestamps().Get<sMultiTensorComponent>();
        REQUIRE( lTimestamps.Shape().CountLayers() == lExpectedTensorLayers );
        REQUIRE( lTimestamps.Shape().mRank == lExpectedTensorRank );
        REQUIRE( lTimestamps.Shape().mShape == lExpectedTensorShape.mShape );
    }
    ...
    SECTION( "Sample single tile" )
    {
        ...
        {
            std::vector<float> lAzimuths0 = lSampler.Retrieve( "Azimuth" ).mValue.FetchFlattened<float>();
        ...
        {
            std::vector<float> lElevations0 = lSampler.Retrieve( "Elevation" ).mValue.FetchFlattened<float>();
